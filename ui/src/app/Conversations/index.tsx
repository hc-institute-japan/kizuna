      ...
      Object.keys(groups).forEach((key: string) => {
        ...
        let messages: Message[] = groups[key].messages
          ? groups[key].messages.map((messageId: string) => {
              let groupMessage: GroupMessage = Object.keys(groupMessagesLocal)
                .length
              ...
              if (isTextPayload(groupMessage.payload)) {
                ...
                let maybeOther: any | undefined =
                  groupMembers[groupMessage.author];
                ...
                if (maybeOther) {
                  ...
                  fileString = String(
                    maybeOther.username +
                      " has sent " +
                      groupMessage.payload.fileName
                  ).toString();
                } else {
                  // MAYBE BUG: assumption is you sent it.
                  // TODO: format for i18n
                  fileString = String(
                    "You sent " + groupMessage.payload.fileName
                  ).toString();
                }
