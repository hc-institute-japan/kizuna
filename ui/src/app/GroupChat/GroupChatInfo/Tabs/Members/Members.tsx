...
import {IonContent, IonIcon, IonItem, IonItemOption, IonItemOptions, IonItemSliding, IonLabel, IonList, IonLoading} from "@ionic/react";
...
const Members: React.FC<Props> = ({ groupId, groupRevisionId}) => {
  ...
  const groupData  = useSelector((state: RootState) => state.groups.conversations[groupId]);
  ...
  return !loading ? (
    <IonContent>
      <IonList className={styles.memberInfo}>
        ...
        <IonItem lines="none" button  onClick={() => setIsOpen(true)} key={"add-members-button"}>
          <IonIcon className={styles.icon} icon={personAddOutline}></IonIcon>
          <IonLabel>Add Members</IonLabel>
        </IonItem>

        <IonItem lines="none" className={styles.memberTitle} key={"members-label"}>
          <h3>Members</h3>
        </IonItem>
          ...
          {members.map((member: any) => {
              return (member.id === groupData.creator) ? (
                <>
                    <IonItemSliding>
                      <IonItem lines="none" key={member.id}>
                        ...
                        <h3>{member.username}<br/>admin</h3>
                        </IonLabel>
                      </IonItem>
                    </IonItemSliding>
                  <br/>
                ...
                <>
                  ...
                  <IonItem lines="none" key={member.id}>
                    <IonLabel className={styles.memberName} key={member.id}>
                      <h3>{member.username}<br/>member</h3>
                    </IonLabel>
                  </IonItem>
                  <IonItemOptions side={"end"}>
                    <IonItemOption onClick={() => handleRemoveMembers(member)} color="danger">REMOVE</IonItemOption>
                  </IonItemOptions>
                  </IonItemSliding>
                </>
