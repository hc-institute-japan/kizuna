...
const ChatDetails: React.FC<Props> = ({ location }) => {
  ...
  const onScrollBottom = (complete: () => Promise<void>, files: P2PMessage[]) => {
  ...
  return (
    <IonPage>
      <IonHeader >
        ...
        <IonToolbar className={styles.controls}>
          <span>
            <IonButtons slot="start">
              <IonBackButton defaultHref={`/u/${state.conversant.username}`} className="ion-no-padding" />
            </IonButtons>
          </span>
        </IonToolbar>
        ...
        <IonToolbar>
          <IonSegment onIonChange={e => handleOnSegmentChange(e.detail.value)}>
      ...
      <IonContent>
        <IonSlides ref={slideRef} className="slides" pager={false} options={slideOpts}>
          <IonSlide>
            <IonGrid>
              <IonRow>
