...
export default (orchestrator, config, installable) => {
  orchestrator.registerScenario("add a contact", async (s, t) => {
    ...
    const res = await bobby_conductor.call('blog', 'post', {body:'hello world'})
  ...
  orchestrator.registerScenario("remove a contact", async (s, t) => {
    ...
    await addContacts("bobby_123")(bobby_conductor);
  ...
  orchestrator.registerScenario("block contact", async (s, t) => {
    ...
    await s.shareAllNodes([alice, bobby, clark]);
