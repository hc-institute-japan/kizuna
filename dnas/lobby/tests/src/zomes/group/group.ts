...
export default (orchestrator, config, installables) => {
  ...
  orchestrator.registerScenario ("create group method test", async(s: ScenarioApi,t) =>{
    ...
    let {content: create_group_content, group_id, group_revision_id} = await createGroup(create_group_input)(alice_conductor);
    await delay(1000);
    ...
    t.deepEqual(create_group_content.name, create_group_input.name, "the group name fields match with the expected value");
    t.deepEqual(create_group_content.creator, alicePubKey, "the group creator fields match with the expected value");
    t.deepEqual(create_group_content.members, [bobbyPubKey], "the group members fields match with the expected value");
  ...
  orchestrator.registerScenario ("add members method AND remove members methods test", async(s,t) =>{
    ...
    let updated_group = await getLatestGroupVersion({group_hash:group_id})(alice_conductor);
    await delay(1000);
    ...
    updated_group = await getLatestGroupVersion({group_hash:group_id})(alice_conductor);
    updated_group.created = original_group_content.created
    await delay(1000);
  ...
  orchestrator.registerScenario ("update group name  method test", async(s,t) =>{
    ...
    let updated_group = await getLatestGroupVersion({group_hash:update_group_name_io.group_id})(alice_conductor);
    updated_group.created = original_group_content.created;
    await delay(1000);
