...
use group_coordinator_types::group_message::{
    GroupMessageContent, GroupMessagesOutput, GroupMsgBatchFetchFilter, PayloadType,
};
...
pub fn get_latest_messages_for_all_groups_handler(
    ...
    let linked_groups_to_the_agent: Vec<EntryHash> = get_links(
        agent_pub_key,
        LinkTypes::AgentToGroup,
        Some(LinkTag::new("member")),
    )?
    .into_iter()
    .map(|link_to_group| -> EntryHash { link_to_group.target.into() })
    .collect();
