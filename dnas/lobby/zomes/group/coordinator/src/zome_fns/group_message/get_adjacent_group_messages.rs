...
use group_coordinator_types::group_message::{
    GroupMessageContent, GroupMessagesOutput, GroupMsgAdjacentFetchFilter, PayloadType,
};
...
pub fn get_adjacent_group_messages_handler(
    ...
    let pivot_path_hash = path_from_str(
        &[filter.group_id.clone().to_string(), days].join("."),
        LinkTypes::GroupHashTimestampPath,
    )?
    .path_entry_hash()?;

    // targets are group message entryhash
    let mut linked_messages =
        get_linked_messages_hash(pivot_path_hash.clone(), PayloadType::All, None, None)?;

    // index of the adjacent message entry hash in the vec of links
    let pivot_index = linked_messages.clone().into_iter().position(|e| {
        e.target
            .eq(&AnyLinkableHash::from(filter.adjacent_message.clone()))
    });
    ...
    if previous_linked_messages.len() < filter.batch_size.into() {
        ...
        {
            let path_hash: EntryHash = children_paths.pop().unwrap().target.into(); // unwrap is safe here (checked that the vector wasnt empty before this)
            let mut new_links = get_linked_messages_hash(path_hash, PayloadType::All, None, None)?;
