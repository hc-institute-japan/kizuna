...
use group_coordinator_types::group_message::{
    GroupMessageContent, GroupMessagesOutput, GroupMsgBatchFetchFilter,
};
...
pub fn get_subsequent_group_messages_handler(
    ...
    if linked_messages.len() < filter.batch_size.into() {
        ...
        while !children_paths.is_empty() && messages_hashes.len() < filter.batch_size.into() {
            let path_hash: EntryHash = children_paths.pop().unwrap().target.into(); // this unwrap is safe (we check the vector wasnt empty before this)
